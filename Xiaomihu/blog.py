...
def blogIndex():
    ...
    blogItems = db.session.query(Blog).join(BlogType,Blog.typeId == BlogType.id).order_by(Blog.addTime.desc()).paginate(int(page), per_page=6, error_out=False)
...
def blogAdmin():
    ...
    if form.validate_on_submit():
        ...
        blogData = Blog(title=title, typeId=typeId, img64base=img64base, content=content,addTime=addTime)
